// <auto-generated />
using System;
using KushBot.Resources.Database;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace KushBot.Migrations
{
    [DbContext(typeof(SqliteDbContext))]
    [Migration("20241110212058_boss-entities-fix")]
    partial class bossentitiesfix
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "8.0.3");

            modelBuilder.Entity("KushBot.DataClasses.Boss", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<long>("AbilitiesValue")
                        .HasColumnType("INTEGER");

                    b.Property<int>("BossBaseIndex")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Health")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Level")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("MessageId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ParticipantSlots")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("Bosses");
                });

            modelBuilder.Entity("KushBot.DataClasses.BossParticipant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("BossId")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("BossId");

                    b.ToTable("BossParticipants");
                });

            modelBuilder.Entity("KushBot.DataClasses.ChannelPerms", b =>
                {
                    b.Property<ulong>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<long>("PermissionsValue")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.ToTable("ChannelPerms");
                });

            modelBuilder.Entity("KushBot.DataClasses.ConsumableBuff", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Duration")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime?>("ExpirationDate")
                        .HasColumnType("TEXT");

                    b.Property<ulong>("OwnerId")
                        .HasColumnType("INTEGER");

                    b.Property<double>("Potency")
                        .HasColumnType("REAL");

                    b.Property<int>("TotalDuration")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("ConsumableBuffs");
                });

            modelBuilder.Entity("KushBot.DataClasses.Infection", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<int>("BapsDrained")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreationDate")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("KillAttemptDate")
                        .HasColumnType("TEXT");

                    b.Property<ulong>("OwnerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("UserInfections");
                });

            modelBuilder.Entity("KushBot.DataClasses.Item", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsEquipped")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Level")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.Property<ulong>("OwnerId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Rarity")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("Items");
                });

            modelBuilder.Entity("KushBot.DataClasses.ItemStat", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<double>("Bonus")
                        .HasColumnType("REAL");

                    b.Property<int>("ItemId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("StatType")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("ItemId");

                    b.ToTable("ItemStats");
                });

            modelBuilder.Entity("KushBot.DataClasses.NyaClaim", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("ClaimDate")
                        .HasColumnType("TEXT");

                    b.Property<string>("FileName")
                        .HasColumnType("TEXT");

                    b.Property<int>("Keys")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("OwnerId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("SortIndex")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Url")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("NyaClaims");
                });

            modelBuilder.Entity("KushBot.DataClasses.Plot", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("AdditionalData")
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("LastActionDate")
                        .HasColumnType("TEXT");

                    b.Property<int>("Level")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("Plots");
                });

            modelBuilder.Entity("KushBot.DataClasses.Quest", b =>
                {
                    b.Property<int?>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsCompleted")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsDaily")
                        .HasColumnType("INTEGER");

                    b.Property<int>("QuestBaseIndex")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("Quests");
                });

            modelBuilder.Entity("KushBot.DataClasses.QuestRequirement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("QuestId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Value")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("QuestId");

                    b.ToTable("QuestRequirements");

                    b.HasDiscriminator<int>("Type");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("KushBot.DataClasses.RarityFollow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Rarity")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.ToTable("RarityFollow");
                });

            modelBuilder.Entity("KushBot.DataClasses.SeasonParameters", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("BlueRoleId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("BossProgress")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("MutedRoleId")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("OrangeRoleId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.ToTable("SeasonParameters");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BlueRoleId = 945785365292285963ul,
                            BossProgress = 1,
                            MutedRoleId = 513478497885356041ul,
                            OrangeRoleId = 1225482353003204719ul
                        });
                });

            modelBuilder.Entity("KushBot.DataClasses.UserEvent", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("BapsChange")
                        .HasColumnType("INTEGER");

                    b.Property<int>("BapsInput")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreationTime")
                        .HasColumnType("TEXT");

                    b.Property<double?>("Modifier")
                        .HasColumnType("REAL");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("UserId", "Type", "CreationTime");

                    b.ToTable("UserEvents");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserPet", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Dupes")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Level")
                        .HasColumnType("INTEGER");

                    b.Property<int>("PetType")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserPets");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserPicture", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("OwnerId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Path")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.HasIndex("Path", "OwnerId")
                        .IsUnique();

                    b.ToTable("UserPictures");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserTutoProgress", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<int>("Page")
                        .HasColumnType("INTEGER");

                    b.Property<int>("TaskIndex")
                        .HasColumnType("INTEGER");

                    b.Property<ulong>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.ToTable("UserTutoProgress");
                });

            modelBuilder.Entity("KushBot.KushBotUser", b =>
                {
                    b.Property<ulong>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Balance")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Cheems")
                        .HasColumnType("INTEGER");

                    b.Property<int>("DailyGive")
                        .HasColumnType("INTEGER");

                    b.Property<int>("DiggerState")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Eggs")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ExtraClaimSlots")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("GoranMaxDigMinutes")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("LastBeg")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LastDestroy")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LastNyaClaim")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LastTylerRage")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LastVendorPurchase")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LastYoink")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("LootedDigger")
                        .HasColumnType("TEXT");

                    b.Property<string>("NyaMarry")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("NyaMarryDate")
                        .HasColumnType("TEXT");

                    b.Property<int>("PetPity")
                        .HasColumnType("INTEGER");

                    b.Property<int>("RageCash")
                        .HasColumnType("INTEGER");

                    b.Property<int>("RageDuration")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("RedeemDate")
                        .HasColumnType("TEXT");

                    b.Property<string>("SelectedPicture")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("SetDigger")
                        .HasColumnType("TEXT");

                    b.Property<int>("TicketMultiplier")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Tickets")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("YikeDate")
                        .HasColumnType("TEXT");

                    b.Property<int>("Yiked")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("KushBot.DataClasses.BapsXQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(0);
                });

            modelBuilder.Entity("KushBot.DataClasses.ChainQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(3);
                });

            modelBuilder.Entity("KushBot.DataClasses.CommandQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(2);
                });

            modelBuilder.Entity("KushBot.DataClasses.CountQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(6);
                });

            modelBuilder.Entity("KushBot.DataClasses.LoseQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(5);
                });

            modelBuilder.Entity("KushBot.DataClasses.ModifierXQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(1);
                });

            modelBuilder.Entity("KushBot.DataClasses.WinQuestRequirement", b =>
                {
                    b.HasBaseType("KushBot.DataClasses.QuestRequirement");

                    b.HasDiscriminator().HasValue(4);
                });

            modelBuilder.Entity("KushBot.DataClasses.BossParticipant", b =>
                {
                    b.HasOne("KushBot.DataClasses.Boss", "Boss")
                        .WithMany("Participants")
                        .HasForeignKey("BossId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Boss");
                });

            modelBuilder.Entity("KushBot.DataClasses.ConsumableBuff", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "Owner")
                        .WithMany("UserBuffs")
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("KushBot.DataClasses.Infection", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "Owner")
                        .WithMany("UserInfections")
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("KushBot.DataClasses.Item", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "Owner")
                        .WithMany("Items")
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("KushBot.DataClasses.ItemStat", b =>
                {
                    b.HasOne("KushBot.DataClasses.Item", "Item")
                        .WithMany("ItemStats")
                        .HasForeignKey("ItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Item");
                });

            modelBuilder.Entity("KushBot.DataClasses.NyaClaim", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "Owner")
                        .WithMany("NyaClaims")
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("KushBot.DataClasses.Plot", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "User")
                        .WithMany("UserPlots")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("KushBot.DataClasses.Quest", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "User")
                        .WithMany("UserQuests")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("KushBot.DataClasses.QuestRequirement", b =>
                {
                    b.HasOne("KushBot.DataClasses.Quest", "Quest")
                        .WithMany("Requirements")
                        .HasForeignKey("QuestId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Quest");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserEvent", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "User")
                        .WithMany("UserEvents")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserPet", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("KushBot.DataClasses.UserPicture", b =>
                {
                    b.HasOne("KushBot.KushBotUser", "Owner")
                        .WithMany("UserPictures")
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("KushBot.DataClasses.Boss", b =>
                {
                    b.Navigation("Participants");
                });

            modelBuilder.Entity("KushBot.DataClasses.Item", b =>
                {
                    b.Navigation("ItemStats");
                });

            modelBuilder.Entity("KushBot.DataClasses.Quest", b =>
                {
                    b.Navigation("Requirements");
                });

            modelBuilder.Entity("KushBot.KushBotUser", b =>
                {
                    b.Navigation("Items");

                    b.Navigation("NyaClaims");

                    b.Navigation("UserBuffs");

                    b.Navigation("UserEvents");

                    b.Navigation("UserInfections");

                    b.Navigation("UserPictures");

                    b.Navigation("UserPlots");

                    b.Navigation("UserQuests");
                });
#pragma warning restore 612, 618
        }
    }
}
