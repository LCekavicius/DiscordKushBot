...
public class TutoBuilder
{
    ...
    private TutorialManager Manager { get; init; }

    private int currentStepId = 0;

    public TutoBuilder(ulong userId, string avatarUrl, TutorialManager tutorialManager, int? selectedPage)
    {
        Manager = tutorialManager;
        UserId = userId;
        Page = selectedPage ?? Math.Min(Manager.GetCurrentUserPage(userId), Manager.PageCount);
    ...
    {
        string strike = (includeEmoji || !Manager.IsPageCompletedForDisplay(UserId, Page)) ? "" : "~~";
    ...
    {
        return page > Math.Min(Manager.GetCurrentUserPage(UserId), Manager.PageCount)
            ? ":x:"
            : Manager.IsStepComplete(UserId, stepIndex, page) ? ":white_check_mark:" : ":purple_circle:";
    }

}

public class Tutorial(TutorialManager tutorialManager) : ModuleBase<SocketCommandContext>
{
    ...
    {
        var builder = new TutoBuilder(Context.User.Id, Context.User.GetAvatarUrl(), tutorialManager, selectedPage);
